...
class BaseCountdown {
    ...
    private int mTimeTextTypefaceIndex;

    public void initStyleAttr(Context context, TypedArray ta) {
        mContext = context;
        isTimeTextBold = ta.getBoolean(com.project.librarycountdown.R.styleable.CountdownView_isTimeTextBold, false);
        ...
        mTimeTextTypefaceIndex = ta.getInteger(R.styleable.CountdownView_timeTextTypefaceIndex, 1);
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    public void setTimeTextTypefaceIndex(int typefaceIndex) {
        mTimeTextTypefaceIndex = typefaceIndex;
        Typeface typeface;
        if (typefaceIndex == TextTypefaceList.typeface_1.getTypefaceIndex()) {
            typeface = Typeface.create("sans-serif", Typeface.NORMAL);
        } else {
            Typeface typefaceFrom = mContext.getResources().getFont(TextTypefaceList.Companion.getTypefaceByIndex(typefaceIndex).getFontId());
            typeface = Typeface.create(typefaceFrom, Typeface.NORMAL);
        }
        mTimeTextPaint.setTypeface(typeface);
    }

    public Integer getTimeTextTypefaceIndex() {
        return mTimeTextTypefaceIndex;
    }
    ...
    protected void initPaint() {
        ...
        Typeface typeface;
        if (mTimeTextTypefaceIndex == TextTypefaceList.typeface_1.getTypefaceIndex()) {
            typeface = Typeface.create("sans-serif", Typeface.NORMAL);
        } else {
            Typeface typefaceFrom = mContext.getResources().getFont(TextTypefaceList.Companion.getTypefaceByIndex(mTimeTextTypefaceIndex).getFontId());
            typeface = Typeface.create(typefaceFrom, Typeface.NORMAL);
        }
        mTimeTextPaint.setTypeface(typeface);
       /* Typeface typeface;
        ...
        //  mTimeTextPaint.setTypeface(mContext.getResources().getFont(R.font.default_font));
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (isShowDay) {
            float s = getDayAndHourContentWidth();
            if (mViewWidth - getDayAndHourContentWidth() > 0) {
                mLeftPaddingSize = (mViewWidth - getDayAndHourContentWidth()) / 2;
            } else {
                mLeftPaddingSize = 0;
            }
            // draw day text
            canvas.drawText(Utils.formatNum(mDay), mLeftPaddingSize + getDayAndHourContentWidth() / 2, mTimeTextBaseline, mTimeTextPaint);
