...
@HiltViewModel
class SignInWorkViewModel @Inject constructor(
    private val signInWorkRepository: SignInWorkRepository,
    private val signInDateRepository: SignInDateRepository,
    savedStateHandle: SavedStateHandle
) : ViewModel() {

    val signInWorkWithDatesList =
        signInWorkRepository.getSignInWorksWithSignInDateByOrderIndexDesc()
    ...
    fun insertSignInDateAndWorkFinalDate(signInDate: SignInDate) {
        viewModelScope.launch {
            ...
            /*signInWorkWithDatesList.value =
                signInWorkRepository.getSignInWorksWithSignInDateByOrderIndexDescNotFlow()*/
        }
    }
    ...
    fun preNullAndUpdateList() {
        val tempList = signInWorkWithDatesList.value
        signInWorkWithDatesList.value = mutableListOf()
        signInWorkWithDatesList.value = tempList

    }

    suspend fun haveFreeNum(): Boolean {
        val freeNum = SignInWork.freeNum
        val currentNum = signInWorkRepository.getSignWorkCount()
        return freeNum > currentNum
    }
}